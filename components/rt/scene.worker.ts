...
export interface BuildResult { triangles: number; nodeCount: number; depth: number; buildMs: number; nodes: ArrayBuffer; packed: ArrayBuffer; materials: Material[]; camera: Scene["camera"]; light?: Scene["light"]; normals?: ArrayBuffer; /** what a playground knows beyond its triangles */ playground?: Pick<Playground, "spawns" | "vehicleMaterials" | "characterMaterial" | "carPaints" | "headLamp" | "tailLamp">; error?: string }
...
self.onmessage = async (event: MessageEvent<BuildRequest>) => {
  ...
  try {
    ...
    const result: BuildResult = { triangles: bvh.triangleCount, nodeCount: bvh.nodeCount, depth: bvh.depth, buildMs, nodes: bvh.nodes, packed: bvh.triangles, materials: scene.materials, camera: scene.camera, light: scene.light, normals: bvh.normals, playground: "spawns" in scene ? { spawns: (scene as Playground).spawns, vehicleMaterials: (scene as Playground).vehicleMaterials, characterMaterial: (scene as Playground).characterMaterial, carPaints: (scene as Playground).carPaints, headLamp: (scene as Playground).headLamp, tailLamp: (scene as Playground).tailLamp } : undefined };
