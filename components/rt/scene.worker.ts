import { buildBvh, cornell, parsePlayground, studio, type Material, type Scene, type StudioOptions } from "@/lib/rt";
...
export interface BuildResult { triangles: number; nodeCount: number; depth: number; buildMs: number; nodes: ArrayBuffer; packed: ArrayBuffer; materials: Material[]; camera: Scene["camera"]; light?: Scene["light"]; normals?: ArrayBuffer; error?: string }
...
self.onmessage = async (event: MessageEvent<BuildRequest>) => {
  ...
  try {
    ...
    const result: BuildResult = { triangles: bvh.triangleCount, nodeCount: bvh.nodeCount, depth: bvh.depth, buildMs, nodes: bvh.nodes, packed: bvh.triangles, materials: scene.materials, camera: scene.camera, light: scene.light, normals: bvh.normals };
