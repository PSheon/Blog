...
export function FieldLab() {
  ...
  useEffect(() => {
    ...
    const px = (x: number) => SIZE / 2 + (x / SPAN) * (SIZE / 2), pz = (z: number) => SIZE / 2 - (z / SPAN) * (SIZE / 2);

    // The real fruit where this slice cuts it, and its average colour for the query points.
    const shape = SHAPES[pair[0]], mean = [0, 0, 0];
    let seed = 11;
    const rng = () => ((seed = (seed * 1664525 + 1013904223) >>> 0) / 2 ** 32);
    for (let i = 0; i < 9000; i++) {
      const p = shape(rng);
      for (let k = 0; k < 3; k++) mean[k] += p[3 + k] / 9000;
      if (Math.abs(p[1]) > SLICE) continue;
    ...
    const n = GRID * GRID, x = new Float64Array(n * DIMS), keep = Math.sqrt(ALPHA_BAR[level]);
    for (let i = 0; i < n; i++) {
    ...
    const eps = getModel().noiseIn(x, level, () => [1, 0]);
  ...
  return (
    <div className="grid gap-4 text-sm">
      ...
      <p className="text-muted-foreground">{steps === 0 ? t.fieldUntrained : t.trainedFor(steps)}</p>
    </div>
  );
}
