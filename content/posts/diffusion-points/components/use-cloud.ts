 ...
 */
export function useCloud(canvas: RefObject<HTMLCanvasElement | null>, count: number, extent: [number, number], frame: (view: CloudView, dt: number) => void, narrowExtent?: [number, number]) {
  ...
  useEffect(() => {
    ...
    return () => { disposed = true; cancelAnimationFrame(raf); seen.disconnect(); view?.dispose(); };
