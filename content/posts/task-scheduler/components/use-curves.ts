...
export type Averaged = { curves: Record<Bar, Float64Array>; /** Mean distance from the truth at each moment, 0–1: averaging the curves themselves lets errors in opposite directions cancel. */ gaps: Record<Bar, Float64Array>; honesty: Record<Bar, Honesty>; total: number; /** Failed attempts per job, and the share of all worker time that went into them. */ failed: number; wasted: number; done: number; of: number };
...
export function useCurves(root: RefObject<HTMLElement | null>, options: JobOptions, workers: number, bars: readonly Bar[], runs = 150): Averaged | null {
  ...
  useEffect(() => {
    ...
    let total = 0, failed = 0, lost = 0, busy = 0;
    const work = () => {
      if (cancelled) return;
      ...
      while (seed <= runs && performance.now() < until) {
        ...
        total += t.total;
        for (const a of t.run.attempts) { busy += a.to - a.from; if (!a.ok) { failed++; lost += a.to - a.from; } }
      ...
      const n = seed - 1;
      setResult({ done: n, of: runs, total: total / n, failed: failed / n, wasted: busy ? lost / busy : 0, gaps: Object.fromEntries(BARS.map((b) => [b, gapSums[b].map((v) => v / n)])) as Record<Bar, Float64Array>, curves: Object.fromEntries(BARS.map((b) => [b, sums[b].map((v) => v / n)])) as Record<Bar, Float64Array>,
