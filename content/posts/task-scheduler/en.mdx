...
No clock ticks. Time only moves when an attempt ends, which is why a job that would take one worker over two hours is scheduled in about 8 microseconds (measured on my machine). The core is two functions and a loop (`Scheduler` in `sim/job.ts`; only the book-keeping lines are left out here):
...
First, the job gets faster and then meets a horizontal line and stops (the upper chart). On average:

| Workers | 1 | 2 | 4 | 8 | 16 and more |
| --- | --- | --- | --- | --- | --- |
| The whole job (minutes) | 122 | 63 | 38 | 31.4 | 31.0 |

Those 31 minutes are the longest chain of tasks waiting on each other; more hands can only wait with it.
...
**Nothing has to start together.** Clusters that train large models often need every member of a job to get a machine at the same moment, or the whole job waits (gang scheduling). Here every task is one worker's.
