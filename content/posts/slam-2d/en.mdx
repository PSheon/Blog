---
title: Drawing the map while finding yourself on it — SLAM from scratch
...
seoTitle: "SLAM from scratch: draw the map, find yourself"
seoDescription: "A small car with no GPS and no map draws the map and finds itself on it at the same time. Drive a lap, watch the map bend, then snap straight."
date: 2026-02-15
updated: 2026-09-20
...
At **a corner**, starting two metres and some fifteen degrees off, it is back in ten steps, to within a millimetre.<Sidenote>The figure's default starts, rerun by a script: at the corner the start is 2.02 m and 14.3° off and ICP stops at step 10; in the corridor the start is 2.40 m off and it stops 0.92 m away, with 75% of points matched, 3.5 cm from the wall. Script and output in `docs/research/slam-2d-icp/`.</Sidenote> Press Scramble a few times: most starts are recovered, and when one is not, ICP knows it, because only one or two points in ten found a partner.
...
With only the wheel springs the chain is slack; any shape will do, so it lies bent the way the wheels said. Press "Close the loop" (recognising an old place is called closing the loop, or loop closure) and one short, stiff spring appears, pulling the two ends of the chain together:
...
- Pose graphs and Gauss–Newton: Grisetti, Kümmerle, Stachniss, Burgard, [*A Tutorial on Graph-Based SLAM*](https://doi.org/10.1109/MITS.2010.939925), IEEE Intelligent Transportation Systems Magazine, 2010.
- How a real 2-D lidar SLAM finds old places over a large area: Hess, Kohler, Rapp, Andor, [*Real-Time Loop Closure in 2D LIDAR SLAM*](https://doi.org/10.1109/ICRA.2016.7487258), ICRA 2016 (Cartographer).
