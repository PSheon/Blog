...
| duty | {PARAMS.decay[3]} (only from {PARAMS.dutyHours[0]}:00 to {PARAMS.dutyHours[1]}:00) | {PARAMS.threshold[3]} | their own office | {PARAMS.recovery[3]} |
...
<Instrument fig="02" title="city / needs" caption="Four lines for four needs, the dashed line the threshold; the darker spans are night. The strip at the bottom is what the person was doing. Untick duty and see how much of the day comes free.">
  <NeedsLab />
</Instrument>

Hunger is the most regular: eight hours to reach 0.6, forty minutes to eat, three meals a day. **Duty** is the one need that is not bodily: it is the pull of work. By day work keeps coming in, so it keeps rising, and only being at the office brings it down; after {PARAMS.dutyHours[1]}:00 nothing new arrives, so whoever has cleared it can leave. Switching it off means nobody in this city has a job to go to.
...
Below is the whole bench. On the right is the overseer: a row per person with what they are doing, where, and how high each need is; choose a row and the camera goes to follow them. Under it the record is written out as sentences. Every knob is here: how many people, the seed, the size of the city, the three heads, duty on or off, how hard people avoid each other.
...
**This does not simulate people.** I chose the four needs and tuned their rates until it looked like a day. Duty starts to build at {PARAMS.dutyHours[0]} in the morning so that it passes its threshold around half past eight; that is not how an office worker works. The model can claim one thing only: without a timetable, a few numbers that rise and fall and one scoring formula are enough for a crowd to find a rhythm.
