...
Lay out 8 × 8 blocks, then decide, in order:

- **Roads.** Every fourth road is a wider arterial, and the east column is the riverside.
- **Zones.** By distance from the centre: commercial nearest, then restaurant streets and more offices, residential outside, a few parks at random (two for seed 1).
- **Height.** A random number times how close the block is to the centre, so the middle is tall and the edges low.

Seed 1 grows 373 buildings: 248 houses, 29 office blocks and 96 shopfronts, the tallest 80 metres.
