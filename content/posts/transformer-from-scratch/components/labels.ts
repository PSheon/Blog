...
const zh = {
  ...
  untrained: "還沒開始訓練。現在的權重是隨機的，所以下面的答案都是亂猜。",
  quiz: "隨堂測驗",
  quizHelp: "這幾題模型在訓練時沒看過。青色是答對的數字，粉紅色是答錯的。點任何一題，下面的注意力圖就會換成那一題。",
  reroll: "換一批題目",
  yourOwn: "自己出一題",
  yourOwnHelp: "剛好 6 個數字。模型只學過這個長度，它的位置表也只有這麼長。",
  right: "全對",
  wrong: "有錯",
...
const en: typeof zh = {
  ...
  untrained: "Not trained yet. The weights are random, so the answers below are guesses.",
  quiz: "Pop quiz",
  quizHelp: "The model never saw these during training. Cyan digits are right, pink ones are wrong. Click any problem to show its attention maps below.",
  reroll: "New problems",
  yourOwn: "Set your own problem",
  yourOwnHelp: "Exactly 6 digits. That is the only length the model was trained on, and its position table is only that long.",
  right: "all correct",
  wrong: "has mistakes",
