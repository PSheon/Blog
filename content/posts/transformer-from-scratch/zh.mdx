...
下面的儀器由上到下是四個步驟：先看清楚任務，按「開始訓練」，看「隨堂測驗」裡的答案從粉紅色（錯）一個個變成青色（對），最後往下看注意力圖，那裡畫的是模型寫每一位數字時正在看輸入的哪裡。

<Instrument fig="01" title="transformer / train" size="wide" caption="選「慢」可以看清楚變化的過程，選「快」大約一兩秒就訓練完成。測驗裡的題目模型在訓練時都沒看過；你也可以在下面自己出一題。">
  <TrainingLab />
</Instrument>

如果一切順利，你會在一百步之內看到三件事同時發生：損失掉到接近零、測驗全部變成青色，以及其中一張注意力圖的下半部長出一條**從右上到左下的斜線**。<Sidenote>我用固定的隨機種子實測：反轉和複製大約在第 30 到 40 步就第一次達到 100%，每一步在我的筆電上約 4 毫秒。哪一個頭負責長出斜線，每次訓練都不一定。</Sidenote>
