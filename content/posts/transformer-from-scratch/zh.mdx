...
按「開始訓練」，然後盯著右下角的兩張注意力圖。

<Instrument fig="01" title="transformer / train" size="wide" caption="任務是把六個數字反過來寫。模型讀到「3 1 4 1 5 9 →」之後，要自己一個字一個字寫出「9 5 1 4 1 3」。青色的數字是答對的，粉紅色是答錯的。">
  <TrainingLab />
</Instrument>

如果一切順利，你會在一百步之內看到三件事同時發生：損失掉到接近零、答對率衝到 100%，以及其中一張注意力圖的下半部長出一條**從右上到左下的斜線**。<Sidenote>我用固定的隨機種子實測：反轉和複製大約在第 30 到 40 步就第一次達到 100%，每一步在我的筆電上約 4 毫秒。哪一個頭負責長出斜線，每次訓練都不一定。</Sidenote>
