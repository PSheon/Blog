...
The instrument below is four steps from top to bottom: look at the task, press Train, watch the answers in the pop quiz turn from pink (wrong) to cyan (right) one by one, and finally look at the attention maps, which show where in the input the model is looking while it writes each digit.

<Instrument fig="01" title="transformer / train" size="wide" caption="Choose “slow” to watch the change happen, or “fast” to finish training in a second or two. The model never saw the quiz problems during training, and you can set one of your own underneath.">
  <TrainingLab />
</Instrument>

If all goes well, three things happen together within a hundred steps: the loss falls to nearly zero, the whole quiz turns cyan, and the lower half of one attention map grows a **diagonal running from upper right to lower left**.<Sidenote>Measured with fixed random seeds: reverse and copy first reach 100% around step 30 to 40, and a step takes about 4 ms on my laptop. Which of the two heads grows the diagonal varies from run to run.</Sidenote>
...
```ts title="content/posts/transformer-from-scratch/components/task.ts"
...
> Position $6 + i$ should read position $5 - i$.
