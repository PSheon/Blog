...
Press Train, and keep your eyes on the two attention maps at the bottom right.

<Instrument fig="01" title="transformer / train" size="wide" caption="The task is to write six digits backwards. After reading “3 1 4 1 5 9 →” the model has to produce “9 5 1 4 1 3” by itself, one token at a time. Cyan digits are right, pink ones are wrong.">
  <TrainingLab />
</Instrument>

If all goes well, three things happen together within a hundred steps: the loss falls to nearly zero, the share of correct answers jumps to 100%, and the lower half of one attention map grows a **diagonal running from upper right to lower left**.<Sidenote>Measured with fixed random seeds: reverse and copy first reach 100% around step 30 to 40, and a step takes about 4 ms on my laptop. Which of the two heads grows the diagonal varies from run to run.</Sidenote>
...
## Why not train a model that talks?
