...
This part is about four things that together make the figure above playable. The result first (my machine, 960×540, full path tracing): 6.7 ms a sample, nearly keeping up with a 120 Hz display while moving (116 to 117 frames a second measured), and in motion a fifth of the noise of "start from nothing every frame".

## One: the world is built once, and what moves only gets new boxes

Part one's BVH took a second to build and then never changed. Here there is a person, five cars, a helicopter and an aeroplane, 11,317 moving triangles in all, and rebuilding every frame is out of the question.
...
**Without WebGPU there is nothing to play.** Part one could show a picture rendered earlier; this one is about things that move, and a still picture would be pointless.
