...
export function schedule(tasks: Task[], workers: number, durations: ArrayLike<number>, from = 0, done?: ArrayLike<number>, begun?: Map<number, number>): Schedule {
  ...
  let free = workers, left = n, now = from;
  if (done) for (let i = 0; i < n; i++) if (done[i] >= 0) { finished[i] = 1; finish[i] = done[i]; left--; }
  if (begun) for (const [id, at] of begun) { start[id] = at; finish[id] = Math.max(now, at + durations[id]); running.push(id); free--; }
  while (left > 0) {
    for (const t of tasks) {
      if (free === 0) break;
      if (finished[t.id] || start[t.id] >= 0 || !t.deps.every((d) => finished[d])) continue;
      start[t.id] = now; finish[t.id] = now + durations[t.id]; worker[t.id] = workers - free; running.push(t.id); free--;
    }
    ...
    now = finish[id]; finished[id] = 1; free++; left--;
  }
